...
function updateAllLayers() {
  ...
  const layer1Values = reluMatrix(
    forwardPass(inputValues, fcWeightLayer1, fcBiasLayer1)
  );
  const normalizedLayer1Values = normalizeArray(layer1Values);
  updateLayerCubeColor(layer1Group, normalizedLayer1Values);

  const layer2Values = reluMatrix(
    forwardPass(layer1Values, fcWeightLayer2, fcBiasLayer2)
  );
